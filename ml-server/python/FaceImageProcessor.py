...
def process_images_from_folder():
    source_folder = r'./face_source'
    destination_folder = r"./results_2"
    
    if not os.path.exists(destination_folder):
        os.makedirs(destination_folder)

    processor = FaceImageProcessor(img_width=4096, img_height=4096)
    ...
    for image_name in loop:
        try:
            if image_name.lower().endswith(('.png', '.jpg', '.jpeg', '.tiff', '.bmp', '.gif')):
                ...
                processed_image = processor.crop_and_resize_image(img)

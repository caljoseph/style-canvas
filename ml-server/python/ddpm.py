...
class DDPM(nn.Module):
    ...
    def sample(self, batch_size=16, return_intermediates=False):
        ...
        return self.p_sample_loop((batch_size, channels, image_height, image_width),
    ...
    def forward(self, img, zt=None):
        device = self.betas.device
        if self.training:
            b = zt.shape[0]  # Batch size should be based on zt
            pred_IPR_list = []
            t = torch.full((b,), self.num_timesteps - 1, device=device, dtype=torch.long)
            noise = torch.randn_like(zt)
            IPR_noisy = self.q_sample(x_start=zt, t=t, noise=noise)
            IPR = IPR_noisy
            c = self.condition(img)  # Extract the IPR (Intermediate Prior Representation)
            ...
            b = img.shape[0]  # Batch size should be based on zt
            shape = (b, self.channels * 4)
            IPR_noisy = torch.randn(shape, device=device)
            c = self.condition(img)  # Extract the IPR (Intermediate Prior Representation)
            IPR = IPR_noisy
            for i in reversed(range(0, self.num_timesteps)):
